...
- **athlete_projection.py is a facade since 2026-07-02** (commit `0a4eb36`). Implementation lives in three modules: `athlete_projection_tables.py` (dataclasses incl. `MixedLMCell`, the six mutable state globals `_COHORT`/`_KM`/`_MIXEDLM`/`_MIXEDLM_CONVERGED_PCT`/`_ENGINE_D_GLOBAL_AVAILABLE`/`_PRECOMPUTED`, precompute/serialize/load, cohort + K-M fitting), `athlete_projection_engine_c.py` (DTOs, `shrinkage_projection`, `to_response_dict`), `athlete_projection_engine_d.py` (MixedLM fitting, `mixed_effects_projection`). `athlete_projection.py` re-exports the public surface via `__all__`; app code keeps importing through the facade. NEVER `from athlete_projection_tables import _COHORT` (or any mutable global) — the fit/load paths REBIND those names, so a from-import goes permanently stale. Read state via the getters (`is_precomputed`, `get_cohort_cell`, `get_mixedlm_cell_count`, ...) or as `tables._COHORT` attribute access. Tests patch state on `backend.app.athlete_projection_tables` (aliased `apt`). `MixedLMCell` lives in tables, not engine D, to avoid a circular import; `precompute_tables` lazy-imports `_fit_mixedlm_cells`.
- **Athlete Projection cohort is 2D (age division x GLP bracket).** `backend/app/athlete_projection_tables.py` precomputes 231 cells at FastAPI startup (7 age divisions x 11 GLP brackets x 3 lifts). Each `GlpCohortCell` stores the mean Huber slope across lifters in that cell. Sparse cells (< 20 lifters) merge upward first, then downward, within the same division. A whole-division fallback kicks in when total n < 20 across all brackets for a lift. The merge + fallback decisions are logged at startup (grep `[athlete_projection] merged`). Cache lookup is `get_cohort_cell(division, bracket, lift)`. Do NOT re-introduce the old `predict(level)` or level-conditioned continuous slope fit -- it was replaced in `045caed` after the Sean Yen pivot.
...
- **Engine D (MixedLM) is wired and live in production at 100% convergence.** Shipped 2026-04-29 in commits `3c3b0c9` (backend) + `0201622` (frontend); gate lowered from 0.90 to 0.70 on 2026-04-30 (`77a00eb`); RE structure simplified to random-intercept-only on 2026-05-01 (`24e12d7`, P5 path 2). `mixed_effects_projection` looks up a `MixedLMCell` per (division, bracket, lift) at runtime; converged cells synthesize a virtual `GlpCohortCell` (kg/day from kg/year, residual from `sqrt(residual_var)`) that drops into Engine C's existing projection math; non-converged cells per-lift fall back to Engine C and stamp `meta.engine_d_partial=True` + `meta.engine_d_fallback_lifts=[...]`. `_ENGINE_D_GLOBAL_AVAILABLE` flips on when live precompute clears `ENGINE_D_GLOBAL_GATE_THRESHOLD = 0.70`. With the simpler RE structure, live full-scale precompute lands at 100% convergence on the 93 cells that meet the n_lifters >= 20 / n_meets >= 60 floor (the 138 floor-skipped cells are unchanged regardless of model). The previous random-slope structure landed at 71.26%. Frontend reads `/api/athlete/projection-engines` via TanStack Query and shows the engine toggle when the gate is on. Per-meet noise (`residual_var`) provides cohort uncertainty; PIs are tighter than the random-slope synthesis but accurate for what the simpler model actually represents. Schema bumped 1->2->3 (old artifacts rejected, fall back to live fit); the 2026-05-01 data-refresh published the v3 artifact to `data-latest`. See memory `feedback_probe_sample_vs_full_scale.md`.
...
- **Backtest is offline-only, but it is not exempt from being correct.** `data/backtest_projection.py` splits every career at 36 months before its final meet, fits on the training side, projects to the exact date of each held-out meet, and buckets by elapsed time into 3/6/12/18/24/36-month horizons. NOT imported by any production module, NOT in CI, but `backend/tests/test_backtest_projection.py` (31 tests) does run in CI, because the artifact it writes is rendered on the About page as fact and is the only evidence behind engine decisions. Ship gates live in `SHIP_GATES` and were rewritten by [ADR 0004](docs/adr/0004-projection-engine-cascade.md): **bias within +/-2pp at 12 and 18 months** leads, then MAPE < 6% at 6mo and < 12% at 12mo, then no paired loss > 2pp to any challenger at 12mo. The old MAPE-only set is superseded because all three of its gates passed on an engine running 5-6% HIGH at both horizons the app serves: a threshold on the size of an error cannot see the direction of it. Requires `data/processed/openipf_global.parquet` (build with `OPENIPF_CSV=<path> python data/preprocess.py --no-scope-filter`).
- **Engine C's projected gain saturates; it is not `slope * t`.** `_effective_days(t, tau)` returns `tau * (1 - exp(-t/tau))`, so gain approaches a ceiling of `slope * tau`, with the instantaneous rate at t=0 still exactly the fitted slope. The constant is `SLOPE_DAMPING_TAU_DAYS` in `constants.py`; `None` restores the old linear behaviour exactly and is a supported value, which is why `project_from_history` needs the `_UNSET_TAU` sentinel to tell "no damping" apart from "caller did not say". Three things to keep in mind when touching this: (1) the segment loop applies the DIFFERENCE of effective times per segment rather than one closed form, so a mid-horizon bracket transition that changes the slope still composes, and with a constant slope the segments telescope back to the closed form (locked by test); (2) the same effective time replaces `t_offset` in the PI's cohort term, because `d(gain)/d(slope)` IS that quantity and a band widening linearly under a saturating mean would claim gains the mean says are impossible; (3) the personal term of the PI still uses raw elapsed time and is therefore deliberately conservative for the damped model.
- **Engine C overshoots, and it is the slope not the level** (measured 2026-08-09, ADR 0004). Signed bias is +5.08% at 12 months and +12.19% at 36, against a 36-month MAPE of 12.85 -- so the error is almost entirely one-directional, not noise. Holding Engine C's own level constant with zero slope is the single most accurate predictor in the whole comparison at every horizon (4.73% MAPE at 12mo, bias +0.84%), which clears the level convention (per-lift max-of-last-3, summed) of blame. Do NOT "fix" this by zeroing the slope: for lifters with <= 8 training meets a flat prediction runs 7.04% LOW at 36 months, so the change has to be damping. The Gompertz cascade was rejected on this evidence, not deferred -- naive persistence captures 5.58 of Gompertz's 5.70pp margin over Engine C at 36 months, and head to head Gompertz is *worse* than persistence there.
- **The backtest scoring pass is ~40 minutes; use `--from-observations` rather than re-running.** Every run dumps `data/backtest_results_observations.jsonl` (gitignored), and `python data/backtest_projection.py --from-observations <dump> --output <artifact>` rebuilds the whole artifact in seconds. Reach for it whenever the change is to a summary statistic, a gate threshold, or a new comparison. The alternative is hand-editing a generated file, which is how an artifact starts disagreeing with the code that generates it. The dump also carries per-observation predictions, so stratified follow-ups (by career stage, training-set size, career-final meets) need no re-run at all.
- **The backtest was nondeterministic and the fix belongs in the harness.** 1,664 lifter-days in the eligible pool carry more than one SBD row, and the production cohort fitter assigns a lifter's GLP bracket from `groupby("Name").tail(1)` over a frame ordered by `(Name, Date)`. With tied sort keys under multi-threaded DuckDB, which row lands last moved between runs, flipping a handful of lifters' brackets and shifting reported MAPE by up to ~0.15pp. `fit_cohort_cells_from_frame` now sorts on a full key and pins `threads=1` with `preserve_insertion_order`. Small next to the effects being measured, but a decision artifact that cannot be reproduced exactly is not worth defending.
- **The backtest must run the engine that ships, and there is exactly one way to keep that true.** Engine C lives in `project_from_history` (`athlete_projection_engine_c.py`); `shrinkage_projection` is a thin loader over it, and the backtest calls the same function with injected `cohort_lookup` / `km_lookup`. Do NOT reimplement the engine in the harness. The pre-2026-08-09 harness did exactly that: it fit the TOTAL series and read the **squat** cohort cell as a "total-level proxy", which understates the cohort slope roughly threefold and uses a level production never computes. Every number the About page had shown since 2026-05-01 described that lookalike, not Engine C. `TestProjectFromHistoryIsTheSameEngine` locks the delegation, and one of its cases is a negative control asserting that an empty `cohort_lookup` actually changes the answer, so a silently-ignored injection cannot pass.
- **Three backtest properties are load-bearing and each was wrong before 2026-08-09.** (1) Cohort cells are fit on TRAINING rows only. Fitting on the full frame leaks every lifter's held-out meets into the slopes used to predict those same meets. (2) Predictions are scored at the held-out meet's REAL date, not at a nominal horizon with a +/- 90 day tolerance. That tolerance was the entire horizon at 3 months and 8% of it at 36, so it injected an error that shrank with horizon and directly corrupted the widening-margin question the backtest exists to answer. (3) Cross-engine comparisons are PAIRED and bootstrapped by lifter. Gompertz declines whenever `curve_fit` fails, so its own-sample MAPE describes an easier subset; and a row-level bootstrap on multi-observation lifters reports an interval ~3x too narrow (measured). Own-sample MAPEs are still published for transparency but are explicitly labelled non-comparable on the About page.
...
- **Muted text is `text-zinc-400`, and that is a contrast floor, not a preference.** On `bg-zinc-950`, zinc-400 measures 7.76:1, zinc-500 measures 4.11:1 and zinc-600 measures 2.57:1 — both below the 4.5:1 WCAG AA minimum for text under 24px, which is essentially all secondary copy here. The code had drifted to 500/600 in 134 places and was swept back on 2026-08-09. The locked colour tokens already said zinc-400; do not "tone down" muted text by dropping a step. `disabled:` variants are exempt (WCAG 1.4.3 excludes inactive components) and are deliberately still zinc-500/600.
- **Whole-page routes must pass `reserveViewport` to every `LoadingSkeleton` in their loading sequence.** `/athlete/{name}` and `/meet/{name}/{date}` replace a ~480 px skeleton with ~2,200 px and ~9,100 px of content respectively, which shoved the footer more than a screen down and scored CLS 0.72 / 0.59. `reserveViewport` holds one viewport of height so the footer starts below the fold and the growth is invisible. **Every skeleton in a route's sequence needs it** — App.tsx's Suspense fallback, the component's query-loading state, and the inner lazy-chunk Suspense fallback — because reserving in one and not the next just moves the shift to the handoff between them.
- **An `aria-label` must CONTAIN the element's visible text (WCAG 2.5.3), or voice control cannot target it.** "Download PNG" answering only to "Download athlete card as PNG" means a user saying "click Download PNG" gets nothing. Four controls failed this. Two fixes: for the home link and MethodPill the `aria-label` was **removed** so the name comes from content and matches by construction (MethodPill's caret is `aria-hidden` so it is not announced); for the download buttons the label was rewritten to lead with the visible text. `MethodPill.test.tsx` locates the trigger by `/^Method ·/i` and has a test asserting the label-in-name property — if you reintroduce an `aria-label` there, that test fails on purpose.
- **Verifying a frontend change is harder than it looks here; three instruments lie.** (1) **Lighthouse cannot audit a protected Vercel preview** — it drops the `_vercel_share` cookie on its own reload and silently audits Vercel's login page instead (tell-tale: `/legal/terms` and "Continue with Google" among the failures). Audit a local `npm run dev`/`preview` with `VITE_API_BASE` pointed at the production backend, and run the SAME audit against reverted sources as a control. (2) **`PerformanceObserver` reads CLS 0 in the in-app browser pane** because the viewport is 0x0 and shifts in a zero-height viewport score zero; always negative-control against a page with a known-bad score. (3) **A 403 from production with `X-Vercel-Mitigated: challenge` is Vercel's bot checkpoint**, triggered by heavy automated curl traffic from one IP — not an outage. Confirm with `web_fetch_vercel_url` (different egress) before reporting the site down.
...
- **`<meta name="description">` is the SEARCH snippet; `og:description` is only the SOCIAL preview.** Both are owned by `ogMeta.ts` as of 2026-08-09. Before that, `description` was missing from `OWNED_META`, so every athlete and meet page served index.html's one site-wide sentence to Google while only the social card was personalized. If you add a new per-page tag, add it to `OWNED_META` or it will be inherited site-wide instead.
- **JSON-LD asserts only what the dataset knows.** `ogMeta.ts` emits `ProfilePage`+`Person` for athletes and `SportsEvent` for meets. Meets deliberately carry **no `location`** (preprocess drops `MeetTown`/`MeetState`, so it is genuinely unknown) and **no `eventStatus`** (they are completed meets; `EventScheduled` would be false). Omitting location forfeits Google Event rich results — that is the accepted cost, do not "fix" it by inventing a venue. JSON-LD is serialized with `<` escaped to `<` so a meet name containing `</script>` cannot break out of the block; meet names are federation-entered free text, so this is a real input and is locked by a test.
- **Query params that index a module dict must be validated at the endpoint, or they 500.** `compute_progression` raises a bare `ValueError` for an unknown `x_axis` or `metric`, which is correct at the module level, but nothing translated it: `GET /api/cohort/progression?x_axis=Furlongs` returned an unhandled 500 with a stack trace on a public endpoint, and the request log showed `CRASH`. Found 2026-08-09 by the first HTTP-level test of these routes. `main._require_choice(param, value, allowed)` now maps them to 422, validating against the module's own constant (`progression.X_AXIS_COLS`, `progression.METRIC_COLS`) so there is one definition of the allowed set. **Deliberately not a blanket `ValueError` handler** — that would relabel genuine internal bugs as client errors and hide them. Any new endpoint that indexes a dict by a query param needs the same call.
- **Not every loose parameter is a bug; several are coerced on purpose, and the test should assert the coercion.** `rankings.metric` falls back to `glp` when unwhitelisted (it is interpolated into `ORDER BY`, so the whitelist is the injection boundary and the coercion is what keeps it safe); `lifters.search_lifters` truncates `q` at 50 chars rather than rejecting; an unknown projection `engine` falls back to Engine C. Asserting a 200 alone would not prove the injected string never reached SQL, so `test_endpoints.py` asserts `metric == "glp"` instead.
- **`TestClient` hides your exception handlers by default.** It is constructed with `raise_server_exceptions=True`, so anything reaching `ServerErrorMiddleware` is re-raised into the test instead of being returned as a response. The `duckdb.Error` handler looked broken for exactly this reason. Use the `raw_client` fixture in `test_endpoints.py` (`TestClient(app, raise_server_exceptions=False)`) whenever asserting on the app's own error envelope.
- **A lifter can enter the same meet twice, so date + meet name is NOT a unique key.** Erik Willis has a Raw entry and a Single-ply entry at Nationals 2020-03-03. Keying the meet-recap picker on `${Date}-${MeetName}` produced a duplicate React key and made the second entry unselectable; it is keyed on array index instead, and the option label carries division AND equipment so the two do not read identically. The e2e smoke suite fails on any console error and every tab stays mounted, so a duplicate key anywhere is a CI failure everywhere. Same class of bug as the About page's paired-table key in session 4.
- **The meet recap is keyed on a MEET, not a year, and that was a data decision.** 62.9% of lifter-years in the Canada+IPF pool contain exactly ONE meet, so a Spotify-style yearly Wrapped would have told most lifters "you did 1 meet", and 2026 had 51 lifters at 2+ meets when it was built. A per-meet card works for 100% of lifters instead of 37%. `isRecappable` in `frontend/src/lib/meetRecap.ts` excludes bombed meets (null total) and non-SBD entries (where `TotalKg` is the partial sum, so a bench-only 90 kg would render under a heading reading "Total"). Do not relax either exclusion: the card is exported and posted publicly, so a wrong number is a public claim about a real person.
- **The roster parser guesses a format, and a wrong guess fails SILENTLY.** It still returns names, they are just the wrong names, and the only symptom is a scouting report where half the field lands in the unranked appendix. Two heuristics in `frontend/src/lib/scoutOverrides.ts` are guarded for this, and both guards were verified to break the naive version first. (1) **"Last, First" detection**: a naive "two comma-separated alphabetic fields" rule turns `Jane Doe, Open` into `Open Jane Doe` and `Jane Doe, Vireo Powerlifting` into `Vireo Powerlifting Jane Doe`. The discriminator is that a surname is ONE token while a full name is two or more, plus `NON_NAME_TOKENS` so a division or sex value can never be read as a given name. (2) **Trailing weight class**: a naive trailing-token strip turns `Anthony Wong #2` into `Anthony Wong`, unranking a real OpenIPF lifter whose `#2` is part of the matched name. Only bare numbers, optionally `kg` or `+`, are stripped. `rosterParse.test.ts` is mostly negative controls plus the real 80-name Sunny Daze roster; do not "simplify" a heuristic without running it.
- **No roster scraper exists, and that is a decision, not a gap** (2026-08-09). SimplMeet, named in the backlog, is not a real powerlifting platform. LiftingCast is real but its `robots.txt` disallows `/meets/*/lifter/*`, `/meets/*/division/*/weightClass/*` and `/meets/*/team/*`, which is exactly the roster data, and it publishes no reachable terms of service. OPA, whose meet the Scout MVP was validated against, publishes no entry list at all. Before proposing a scraper again, read the Scout Phase 2 section of NEXT_STEPS.
- **Search dropdowns must say when they truncated.** Lifter Lookup requests 25 and Athlete Projection requests `SEARCH_LIMIT` (12); both render a "showing the top N by best total" note when the result count hits the limit. Common surnames blow past this ("Lee" matches ~199 lifters), and without the note a user scrolls to the end of a full list and concludes they are not in the dataset. The limit is passed into `SelectorSearch` as a prop rather than hardcoded so the notice cannot drift from the fetch. `search_lifters` really does `ORDER BY best_total DESC`, so "top N by best total" is accurate — keep them in sync if that ORDER BY changes.
...
- `cd cpu-analytics && .venv/Scripts/python -m pytest backend/tests/ -v` --
  519 backend tests, 1 skipped, ~80 s, covering every API endpoint at the HTTP
  layer (`test_endpoints.py`: happy path plus a rejected input each, ETag/304,
  HEAD on the probes, the duckdb 503 envelope), rankings, meets, progression (incl.
...
- `cd frontend && npm run test` -- 140 Vitest unit tests (incl. roster-format
  parsing in `rosterParse.test.ts`) (ogMeta incl. JSON-LD + route + percentile + useUrlState
