...
- **Engine D (MixedLM) is wired but currently gated off in production.** Shipped 2026-04-29 in commits `3c3b0c9` (backend) + `0201622` (frontend). `mixed_effects_projection` looks up a `MixedLMCell` per (division, bracket, lift) at runtime; converged cells synthesize a virtual `GlpCohortCell` (kg/day from kg/year, residual from `sqrt(random_slope_var)`) that drops into Engine C's existing projection math; non-converged cells per-lift fall back to Engine C and stamp `meta.engine_d_partial=True` + `meta.engine_d_fallback_lifts=[...]`. `_ENGINE_D_GLOBAL_AVAILABLE` flips on only when live precompute clears `ENGINE_D_GLOBAL_GATE_THRESHOLD = 0.90`. Frontend reads `/api/athlete/projection-engines` via TanStack Query and shows the engine toggle only when the gate is on. **Toggle is currently hidden** because the full-scale precompute lands at 71-72% convergence (vs the N=200 probe's 91.7%), which is below the 90% gate. Two ways to unhide: lower the threshold to ~0.70, or simplify the random-effects structure to random-intercept-only. See memory `feedback_probe_sample_vs_full_scale.md`. Schema bumped 1->2 (old artifacts rejected, fall back to live fit); next data refresh republishes v2 to `data-latest`.
