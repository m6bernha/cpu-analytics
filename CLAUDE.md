...
- **Provincial landscape** (2026-04-22 audit complete, 10/10 provinces): Ontario shipped (Phase 2). Five remaining scraper targets: Alberta (JPG + OCR), Manitoba (PDF), Newfoundland (.docx, stale 2022), Quebec (JS-rendered, needs Playwright), Nova Scotia (Google Sheet, 0.9 * CPU rounded up 2.5 kg -- do NOT derive from CPU). Three routing-only: British Columbia + Saskatchewan reuse CPU Regional Western; New Brunswick + PEI are open-entry with no published QT. See NEXT_STEPS.md "Phase 3+ remaining provinces" for the full decision matrix, spot-check values per scraper, and build order (AB -> MB -> NL -> QC -> NS). Per-province work is its own session using `opa.py` as the template.
