...
- **Engine D (MixedLM) is wired and live in production behind a 70% convergence gate.** Shipped 2026-04-29 in commits `3c3b0c9` (backend) + `0201622` (frontend); gate lowered from 0.90 to 0.70 on 2026-04-30. `mixed_effects_projection` looks up a `MixedLMCell` per (division, bracket, lift) at runtime; converged cells synthesize a virtual `GlpCohortCell` (kg/day from kg/year, residual from `sqrt(random_slope_var)`) that drops into Engine C's existing projection math; non-converged cells per-lift fall back to Engine C and stamp `meta.engine_d_partial=True` + `meta.engine_d_fallback_lifts=[...]`. `_ENGINE_D_GLOBAL_AVAILABLE` flips on when live precompute clears `ENGINE_D_GLOBAL_GATE_THRESHOLD = 0.70`. Live full-scale precompute lands at 71-72% convergence (vs the N=200 probe's 91.7%), so the gate is open but ~29% of cells silently fall back to Engine C — `meta.engine_d_partial` identifies the affected projections per response. Frontend reads `/api/athlete/projection-engines` via TanStack Query and shows the engine toggle when the gate is on. The principled v2 follow-up is to simplify the random-effects structure to random-intercept-only (P5 path 2 in NEXT_STEPS.md); see memory `feedback_probe_sample_vs_full_scale.md`. Schema bumped 1->2 (old artifacts rejected, fall back to live fit); next data refresh republishes v2 to `data-latest`.
