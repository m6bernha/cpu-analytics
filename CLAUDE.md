...
- **Engine D (MixedLM) toggle is gated off.** `mixed_effects_projection` delegates to shrinkage and stamps `meta.engine_d_available=false`. The frontend `AthleteProjection.tsx` hides the Simple/Advanced radio via `{false && <EngineToggle ...>}` until the MixedLM precompute + convergence probe lands. Full unbundling ships in a follow-up commit per NEXT_STEPS.
...
- **Athlete Projection QT reference lines.** Optional on the Total view only (per-lift QTs do not exist). Reuses `fetchQtStandards()` + the Lifter Lookup `findQtForLifter` pattern. Era picker defaults to `2027` because projections point forward. Recharts v3 defaults `ifOverflow` to `discard` on ReferenceLine, which silently drops lines whose y is outside the auto-domain; `ifOverflow="extendDomain"` is load-bearing and must not be removed. Landed in commit `8452835`.
- **Precompute startup cost.** The 231-cell cohort fit + 7 K-M tables take ~27 s on the Canada+IPF parquet. The `[startup] athlete_projection tables: cohort_cells=... km=... elapsed_ms=...` log line in `backend/app/main.py` surfaces the duration on every cold start so regressions are trivial to spot. UptimeRobot keepalive keeps Render warm between 5 min pings, so real users almost always hit a warm backend. If keepalive ever misses or the user base grows, the cleaner fix is serializing cohort cells at preprocess time (load from disk on boot, ~ms) rather than lazy precompute on first projection request (risks 27 s for the unlucky first user after every cold start).
