...
- **Provincial landscape** (2026-04-22 audit, partial): Ontario confirmed separate and shipped. British Columbia confirmed reuses CPU Regional QTs (their provincial page explicitly points to powerlifting.ca). Alberta likely has its own numbers but the site returned ECONNREFUSED during the audit. Quebec's FQD page is JS-rendered and WebFetch couldn't parse it. SK / MB / NB / NS / PE / NL not yet audited. See NEXT_STEPS.md "Phase 3+" for the full table. Per-province work is its own session -- audit first, then build a scraper module only if the numbers actually differ.
