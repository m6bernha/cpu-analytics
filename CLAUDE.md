...
- **Live QT frontend MVP** at `frontend/src/tabs/QtLiveCoveragePanel.tsx`, rendered above the existing four-block view in `QTSqueeze.tsx`. Filter panel: Sex, Level, Division, Effective Year, Region (conditional on 2027 Regionals). Shows per-weight-class coverage table plus `fetched_at` date. Degrades to a one-line banner when `live_data_available: false`. The full UX rebuild (retiring the 4-block layout entirely) is pending — see NEXT_STEPS.md "QT Squeeze overhaul" section.
